...
public class ObjectMove : MonoBehaviour
{
    ...
    void HitObj(RaycastHit hit)
    {
        ...
        m_makedObject.SetActive(true);
        Destroy(m_makedObject, DestroyTime2);
    }

    public void HitObj(Transform transform)
    {
        var pos = new Vector3(transform.position.x, 1, transform.position.z);
        m_makedObject = Instantiate(m_hitObject, pos, Quaternion.LookRotation(Vector3.forward)).gameObject;
        m_makedObject.SetActive(true);
        Destroy(m_makedObject, DestroyTime2);
    }
